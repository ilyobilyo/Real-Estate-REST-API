...
namespace bgbrokersapi.Controllers.Admin
{
    [Route("/SellType")]
    [ApiController]
    public class SellTypeController : TypeController
    {
        public SellTypeController(ITypeService typeService,
             IMapper mapper)
        ...
        {
            var response = await typeService.Create<SellType>(model);
        ...
        {
            var response = await typeService.DeleteType<SellType>(id);
        ...
        public async override Task<IActionResult> GetById([FromRoute] int id)
        {
            ...
            try
            {
                var sellType = await typeService.GetById<SellType>(id);

                response.Status = StatusCodes.Status200OK;
                response.Type = mapper.Map<TypeModel>(sellType);
            }
            catch (Exception e)
            {
        ...
        public override async Task<IActionResult> GetAll()
        {
            ...
            var types = await typeService.GetAll<SellType>();
        ...
        {
            var response = await typeService.UpdateType<SellType>(id, model);
