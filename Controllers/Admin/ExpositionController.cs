...
namespace bgbrokersapi.Controllers.Admin
{
    [Route("/Exposition")]
    [ApiController]
    public class ExpositionController : TypeController
    {
        public ExpositionController(ITypeService typeService,
             IMapper mapper)
        ...
        {
            var response = await typeService.Create<Exposition>(model);
        ...
        {
            var response = await typeService.DeleteType<Exposition>(id);
        ...
        public async override Task<IActionResult> GetById([FromRoute] int id)
        {
            ...
            try
            {
                var exposition = await typeService.GetById<Exposition>(id);

                response.Status = StatusCodes.Status200OK;
                response.Type = mapper.Map<TypeModel>(exposition);
            }
            catch (Exception e)
            {
        ...
        public override async Task<IActionResult> GetAll()
        {
            ...
            var types = await typeService.GetAll<Exposition>();
        ...
        {
            var response = await typeService.UpdateType<Exposition>(id, model);
